...
const techIcons: Record<string, JSX.Element> = {
  ...
  Tailwind: <TailwindIcon className="w-8 h-8 md:w-12 md:h-12" />,
...
export default function ProjectsCard({project}: {project: Project}) {
  return (
    <div className="bg-black/70 rounded-xl text-center m-1 w-full relative">
      <div className="p-2">
        <span className="text-xl md:text-3xl font-bold">{project.name}</span>
      </div>
      ...
      <div className="flex flex-row gap-4 p-1 md:p-2 justify-around">
        {project.techStack.map(tech => (
          <div className="flex flex-col items-center" key={tech}>
            <div>{techIcons[tech]}</div>
            <div><span className="font-bold text-xs md:text-sm">{tech}</span></div>
          </div>
        ))}
      </div>
